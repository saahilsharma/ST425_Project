...
def plot_confusion_matrix(cm, title):
    ...
    ax.xaxis.set_ticklabels(['Did not die', 'Died'])
    ax.yaxis.set_ticklabels(['Did not die', 'Died'])
    plt.show()


def fit_ml_algo(algo, name, X_train, Y_train, X_test, Y_test, cv, storage):
    # Focus, on precision score, recall score
...
storage = {'Train F1' : [], 'Train precision': [], 'Train recall' : [],
           'Test F1': [], 'Test precision' :[],
          'Test recall' : [], 'Name' : []}


fit_ml_algo(LogisticRegression(class_weight = 'balanced'), 'LR',
            X_train, Y_train,
            X_test, Y_test, 10, storage)

fit_ml_algo(LinearDiscriminantAnalysis(), 'LDA',
            X_train, Y_train,
            X_test, Y_test, 10, storage)

fit_ml_algo(SVC(kernel='linear', class_weight = 'balanced'),
            'SVM Linear with C = 1 (Default)', X_train, Y_train,
            X_test, Y_test, 10, storage)

C_f1 = find_optimized_tuining_param(list(range(1, 40)))
fit_ml_algo(SVC(kernel='linear', C = C_f1, class_weight = 'balanced'),
            "SVM Linear with C = " + str(C_f1), X_train, Y_train,
            X_test, Y_test, 10, storage)
